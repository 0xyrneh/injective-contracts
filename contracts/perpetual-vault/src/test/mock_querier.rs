    ...
    F: FnOnce(&mut WasmMockQuerier),
{
    let mut custom_querier: WasmMockQuerier =
        WasmMockQuerier::new(MockQuerier::new(&[(TEST_CONTRACT_ADDR, contract_balance)]));
    ...
    pub fn handle_query(&self, request: &QueryRequest<InjectiveQueryWrapper>) -> QuerierResult {
        match &request {
            ...
            QueryRequest::Custom(query) => match query.query_data.clone() {
                InjectiveQuery::DerivativeMarket { market_id } => {
                    match &self.perpetual_market_response_handler {

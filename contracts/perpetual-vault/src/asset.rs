...
pub fn format_lp_token_name(denom: &String) -> StdResult<String> {
    let short_denom: String = denom.chars().take(TOKEN_SYMBOL_MAX_LENGTH).collect();
    Ok(format!("{}-LP", short_denom).to_uppercase())
}
